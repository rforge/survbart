...
mc.surv.bart <- function(
    ...
    verbose = TRUE,
    seed = 99L,   ## required for mc.surv.bart only
    mc.cores = 2L ## required for mc.surv.bart only
)
{
    ...
    for(i in 1:mc.cores) {
        mcparallel(surv.bart(x.train=x.train, y.train=y.train, x.test=x.test,
                        k=k,
...
## N.B. $binaryOffset/$call, if present, are from the first posterior only
